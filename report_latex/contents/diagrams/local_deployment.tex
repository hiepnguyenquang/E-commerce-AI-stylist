...
\begin{tikzpicture}
    \node[diagram user, minimum width=3.0cm] (browser) at (0,0) {Browser};
    ...
    \node[diagram data, minimum width=3.0cm] (postgres) at (4.8,-3.4) {PostgreSQL 15\\port 5432};
    \node[diagram data, minimum width=2.8cm] (redis) at (8.5,-3.4) {Redis\\port 6379};
    \node[diagram queue, minimum width=3.1cm] (rabbit) at (12.2,-3.4) {RabbitMQ\\5672 / 15672};
    \node[diagram data, minimum width=3.1cm] (lance) at (16.1,-3.4) {LanceDB\\vector local};
    \node[diagram data, minimum width=3.1cm] (files) at (20.0,-3.4) {Static files\\VTON outputs};
    ...
    \begin{scope}[on background layer]
        \node[diagram group, fill=gray!2, fit=(web)(medusa)(fastapi)(postgres)(redis)(rabbit)(lance)(files),
              label={[font=\small\bfseries]above left:Môi trường phát triển local}] {};
        \node[diagram group, fill=violet!2, fit=(gemini)(replicate),
              label={[font=\small\bfseries]above:Dịch vụ cloud}] {};
    \end{scope}
...
\caption{Sơ đồ triển khai hệ thống trong môi trường phát triển local}
